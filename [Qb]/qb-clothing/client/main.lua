                        ...
                        if dist < 5 then
                            ...
                            if IsControlJustPressed(0, 38) then -- E
                                if Config.Stores[k].shopType == "clothing" then
                                    ...
                                    openMenu({
                                        {menu = "character", label = "Kläder", selected = true},
                                        {menu = "accessoires", label = "Tillbehör", selected = false}
                                    })
                                elseif Config.Stores[k].shopType == "barber" then
                                    customCamLocation = nil
                                    openMenu({
                    ...
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                DrawText3Ds(Config.ClothingRooms[k].x, Config.ClothingRooms[k].y, Config.ClothingRooms[k].z + 0.3, '~g~E~w~ - Visa kläder')
                                if IsControlJustPressed(0, 38) then -- E
                                    ...
                                    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            {menu = "roomOutfits", label = "Förinställningar", selected = true, outfits = Config.Outfits[PlayerData.job.name][gender]},
                                            {menu = "myOutfits", label = "Mina kläder", selected = false, outfits = result},
                                            {menu = "character", label = "Kläder", selected = false},
                                            {menu = "accessoires", label = "Tillbehör", selected = false}
                                        })
...
AddEventHandler('qb-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({
            {menu = "myOutfits", label = "Mina kläder", selected = true, outfits = result},
        })
    end)
end)
...
AddEventHandler('qb-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
        {menu = "character", label = "Karaktär", selected = true},
