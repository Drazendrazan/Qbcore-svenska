...
local function OnPlayerConnecting(name, setKickReason, deferrals)
    ...
    deferrals.update(string.format("Hej %s. Ditt Steam-ID kontrolleras.", name))
    ...
    deferrals.update(string.format("Hej %s. Vi kollar så du inte är bannad.", name))
	
    local isBanned, Reason = QBCore.Functions.IsPlayerBanned(player)
	
    Wait(2500)
	
    deferrals.update(string.format("Välkommen %s till {Server Name}.", name))

    if not steamIdentifier then
        deferrals.done("Du är inte anslutet till steam.")
...
AddEventHandler('QBCore:server:CloseServer', function(reason)
    ...
    if QBCore.Functions.HasPermission(source, "admin") or QBCore.Functions.HasPermission(source, "god") then 
		...
		QBCore.Functions.Kick(src, "Du har inte behörighet för detta..", nil, nil)
    end
end)
...
AddEventHandler('QBCore:UpdatePlayer', function(data)
	...
	if Player ~= nil then
		...
		TriggerClientEvent('QBCore:Notify', src, "Du fick din lönecheck på $"..Player.PlayerData.job.payment)
			...
			if Player ~= nil then
				...
				if (QBCore.Functions.HasPermission(source, "god") or QBCore.Functions.HasPermission(source, QBCore.Commands.List[command].permission)) then
					if (QBCore.Commands.List[command].argsrequired and #QBCore.Commands.List[command].arguments ~= 0 and args[#QBCore.Commands.List[command].arguments] == nil) then
					    TriggerClientEvent('QBCore:Notify', source, "Alla argument måste fyllas i!", "error")
					    local agus = ""
					...
					TriggerClientEvent('QBCore:Notify', source, "Inte tillgång till detta kommando", "error")
				end
			end
		end
	end
...
AddEventHandler('QBCore:ToggleDuty', function()
	...
	if Player.PlayerData.job.onduty then
		...
		TriggerClientEvent('QBCore:Notify', src, "Du är nu ledig!")
	else
		Player.Functions.SetJobDuty(true)
		TriggerClientEvent('QBCore:Notify', src, "Du är nu i tjänst!")
	end
