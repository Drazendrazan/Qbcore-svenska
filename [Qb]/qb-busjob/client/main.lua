...
for i=1,Lijnen,1 do
	items["buslijn"..i]={cb=function(name, item, menu) 
		Citizen.CreateThread(function() 
			...
			if SpawningBus then
				JobsClientCore.DisplayNotification("Din buss är på väg!")
				return false
			end
			SpawningBus = true
...
Citizen.CreateThread(function() 
	...
	table.insert(JobsClientCore.MarkerList, 
		{name="Bus_Kleedkamer",
			...
			MarkerHelpTekst="",
			onClick=function(marker) print("Tryck E") end,
	...
	table.insert(JobsClientCore.MarkerList, 
		{name="Bus_DelVeh",
			...
			r=204,g=50,b=50,
			MSizex=1.5,MSizey=1.5,MSizez=10,
			MarkerHelpTekst="Ta bort din buss",
...
function RunRoute(Route)
	...
	JobsClientCore.RunRoute(Route, {r=189,g=214,b=69,Blip=24,cat=2}, 20.5, 50.5,
		...
		"Du kan bara fortsätta om du är i din buss!", 
		...
		function(location) --At location
			...
			AddTextComponentString("~INPUT_DETONATE~ Dörr öppen")
			...
			if CheckDistance(location) then
				...
				SetTextComponentFormat('STRING')
				AddTextComponentString("~INPUT_DETONATE~ Stäng dörrar")
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
	...
	if (IsAnyVehicleNearPoint(marker.x, marker.y, marker.z, 10.50) == false) then
		Citizen.CreateThread(function()
			...
			if IsWorking_Bus then
				...
				JobsClientCore.DisplayNotification("Du måste ta på dig rätt outfit!")
			end
		end)
	else
		JobsClientCore.DisplayNotification("Det finns något nära spawn-punkten!!")
	end
end
