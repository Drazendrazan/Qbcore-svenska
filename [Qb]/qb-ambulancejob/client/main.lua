        ...
        if QBCore ~= nil then
            ...
            if #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 1.5 then
                if doctorCount >= Config.MinimalDoctors then
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Ring läkare")
                else
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Checka in")
                end
                    ...
                    if doctorCount >= Config.MinimalDoctors then
                        ...
                        QBCore.Functions.Progressbar("hospital_checkin", "Checking in..", 2000, false, true, {
                            ...
                            if bedId ~= nil then 
                                ...
                                QBCore.Functions.Notify("Sängarna är upptagna..", "error")
                            end
                        end, function() -- Cancel
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                            QBCore.Functions.Notify("Det gick inte att checka in!", "error")
                        end)
                    end
                end
            elseif #(pos - vector3(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z)) < 4.5 then
                if doctorCount >= Config.MinimalDoctors then
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Ringa upp")
                else
                    QBCore.Functions.DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "Checka in")
                end
            end
            ...
            if closestBed ~= nil and not isInHospitalBed then
                if #(pos - vector3(Config.Locations["beds"][closestBed].x, Config.Locations["beds"][closestBed].y, Config.Locations["beds"][closestBed].z)) < 1.5 then
                    QBCore.Functions.DrawText3D(Config.Locations["beds"][closestBed].x, Config.Locations["beds"][closestBed].y, Config.Locations["beds"][closestBed].z + 0.3, "~g~E~w~ - Att ligga i sängen")
                    if IsControlJustReleased(0, 38) then
    ...
    while true do
        ...
        if QBCore ~= nil then
            if isInHospitalBed and canLeaveBed then
                local pos = GetEntityCoords(PlayerPedId())
                QBCore.Functions.DrawText3D(pos.x, pos.y, pos.z, "~g~E~w~ - Att ta sig ur sängen..")
                if IsControlJustReleased(0, 38) then
                    LeaveBed()
                end
            else
...
AddEventHandler('hospital:client:Revive', function()
    ...
    QBCore.Functions.Notify("Du är helt frisk igen!")
end)
...
AddEventHandler('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    QBCore.Functions.Notify("Dina sår har läkts!")
end)
...
AddEventHandler('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    Citizen.CreateThread(function ()
        Citizen.Wait(5)
        if isRevive then
            QBCore.Functions.Notify("Du blir hjälpt..")
    ...
    if not isDead then
        if #injured > 0 then
            ...
            if #injured <= Config.AlertShowInfo then
                ...
                limbDamageMsg = "Du har ont på många ställen.."
            end
            QBCore.Functions.Notify(limbDamageMsg, "primär", 5000)
        end
    end
end
...
function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        QBCore.Functions.Notify("Du är "..Config.BleedingStates[tonumber(isBleeding)].label, "error", 5000)
    end
end
...
function MenuOutfits()
    MenuTitle = "Outfits"
    ClearMenu()
    Menu.addButton("Mina kläder", "OutfitsLijst", nil)
    Menu.addButton("Stäng menyn", "closeMenuFull", nil) 
end
...
function OutfitsLijst()
    QBCore.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        MenuTitle = "Mina kläder :"
        ClearMenu()

        if outfits == nil then
            QBCore.Functions.Notify("Du sparade inga kläder...", "error", 3500)
            closeMenuFull()
        else
            for k, v in pairs(outfits) do
                Menu.addButton(outfits[k].outfitname, "alternativMeny", outfits[k]) 
            end
        end
        Menu.addButton("Tillbaks", "MenuOutfits",nil)
    end)
end
...
function optionMenu(outfitData)
    ...
    Menu.addButton("Välj Outfit", "selectOutfit", outfitData) 
    Menu.addButton("Ta bort outfit", "removeOutfit", outfitData) 
    Menu.addButton("Tillbaks", "OutfitsLijst",nil)
end

function selectOutfit(oData)
    TriggerServerEvent('clothes:selectOutfit', oData.model, oData.skin)
    QBCore.Functions.Notify(oData.outfitname.." valt", "success", 2500)
    closeMenuFull()
    changeOutfit()
end
...
function removeOutfit(oData)
    ...
    QBCore.Functions.Notify(oData.outfitname.." har blivit raderad", "success", 2500)
    closeMenuFull()
end
