                    ...
                    if dist <= 7.5 then
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                DrawText3Ds(Config.Locations[k]["coords"]["x"], Config.Locations[k]["coords"]["y"], Config.Locations[k]["coords"]["z"], '~g~E~w~ - Tvättar bil (kr'..Config.DefaultPrice..')')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent('qb-carwash:server:washCar')
                                end
                            else
                                DrawText3Ds(Config.Locations[k]["coords"]["x"], Config.Locations[k]["coords"]["y"], Config.Locations[k]["coords"]["z"], 'Biltvätt är inte tillgänglig ..')
                            end
                        end
                    end
                end
...
AddEventHandler('qb-carwash:client:washCar', function()
    ...
    QBCore.Functions.Progressbar("search_cabin", "Fordonet håller på att tvättas ..", math.random(4000, 8000), false, true, {
        ...
        QBCore.Functions.Notify("Tvätt inställd ..", "error")
        washingVehicle = false
    end)
end)
