        ...
        if IsControlJustPressed(0, Config.OpenPhone) then
            if not PhoneData.isOpen then
                ...
                if not IsHandcuffed then
                    ...
                    QBCore.Functions.Notify("GPS har ställts in..", "error")
                end
            end
        end
        Citizen.Wait(3)
...
function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.Notify("Du har ingen telefon", "error")
        end
    end)
end
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    QBCore.Functions.Notify('GPS-plats inställd: '..data.alert.title)
...
AddEventHandler('qb-phone:client:RemoveBankMoney', function(amount)
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Bank",
                text = "Kr"..amount..",- har tagits bort från ditt saldo!", 
                icon = "fas fa-university", 
                color = "#ff002f",
                timeout = 3500,
            },
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Bank",
                content = "Kr"..amount..",- har tagits bort från ditt saldo!", 
                icon = "fas fa-university",
                timeout = 3500, 
                color = "#ff002f",
            },
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Du är för långt borta från loppet. GPS är inställd på loppet.', 'error', 5000)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("GPS har satts på " .. data.HouseData.HouseData.adress .. "!", "success")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS har ställts in!', 'success')
end)
